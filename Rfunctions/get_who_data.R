...
country.level.variables <- w[w$date == max(w$date),c("Country","ISO_2_CODE","ISO_3_CODE","iso3code","ADM0_NAME","date_epicrv")]
full.time.series <- expand.grid(Country=unique(w$Country), date=unique(w$date))
full.time.series <- merge(full.time.series, country.level.variables, by="Country")
w <- merge(full.time.series, w[c("Country","date","NewCase","CumCase","NewDeath","CumDeath")], by=c('Country','date'), all.x=TRUE)
w$NewCase[is.na(w$NewCase)] <- 0
w$CumCase[is.na(w$CumCase)] <- 0
w$NewDeath[is.na(w$NewDeath)] <- 0
w$CumDeath[is.na(w$CumDeath)] <- 0
