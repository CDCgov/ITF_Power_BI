...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
hotspot_data <- function(){
  
  df.Countries.Daily <- fun_jhu() 
  
#consolidate the df.Countries.Daily dataset as some areas have multiple observations
df.Countries.Daily.cons <- df.Countries.Daily[c("Country","Date","Cumulative Cases","Cumulative Deaths")] %>%
  group_by(Country,Date) %>% 
  dplyr::summarize(Cumulative_Cases = sum(`Cumulative Cases`,na.rm = TRUE),
            Cumulative_Deaths = sum(`Cumulative Deaths`,na.rm = TRUE)) %>% 
  ungroup()

#subset out the WHO regions
#WHO_reg <- df.Countries.Daily[c("Country","WHO Region","Population 2018")] %>% 
WHO_reg <- df.Countries.Daily[c("Country","WHO Region","Population 2018.x")] %>% 
  group_by(Country,`WHO Region`,`Population 2018.x`) %>% dplyr::summarize() %>% 
  ungroup()
# WHO_reg


## CASES
df.cases_long <- df.Countries.Daily.cons[c("Country","Date","Cumulative_Cases")]
df.cases <- df.cases_long %>%  spread(Date, `Cumulative_Cases`) 
df.cases_fin <- full_join(WHO_reg,df.cases,by="Country")
# head(df.cases_fin)

#DEATHS
df.deaths_long <- df.Countries.Daily.cons[c("Country","Date","Cumulative_Deaths")]
df.deaths <- df.deaths_long %>% spread(Date, `Cumulative_Deaths`) 
df.deaths_fin <- full_join(WHO_reg,df.deaths,by="Country")
...
#empty = which(is.na(scount[1,]))[1]
#if(!is.na(empty)) {
#  scount = scount[,-(empty:ncol(scount))]
#}
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
ccount$id <- 1:nrow(ccount)
  ...
  ccount <- ccount %>% 
    select(Country:`Population 2018.x`,Type, id, everything()) %>% 
    # convert NAs to zero
...
# Remove contries with no cases ever 
  keep <- cinc$Country[apply(ccount[,c(6:ncol(ccount))], 1, sum) != 0]
# View(keep)
...
#  cinc = cinc[cinc$id %in% keep,]
  cincx = cinc[cinc$Country %in% keep,]
...
for(r in 1:nrow(delta)) {
  ...
  rin = as.numeric(cum3[cum3$id==delta$id[r],-(1:5)])
  df = data.frame(name = delta$Country[r],
                  id = delta$id[r],
                  dates = covid.dates,
...
# Get datasets needed in function
hotspot_data()
# fdf <- hotspot("Case",
#                  10,
#                  0.1,
...
alldata <- dplyr::bind_rows(purrr::map2(.x = sort(rep(c(5,10,15,20),4)), 
                     .y = c(rep(c(.01,.05,.1,.5),4)), .f = ~hotspot("Case", .x, .y, 100)))

frame_df <- fun_frame()

alldatax <- alldata %>% rename(country = name,
                               date = dates)


allthedata <- left_join(alldatax, frame_df) %>% 
  #Creating variable that is not date dependent
  mutate(ou_cut_match = paste(country_code, slope_cut, rate_cut, sep="_"))

gisdata <- allthedata %>% select(-ou_date_match) %>% 
