...
df.cases <- pivot_wider(df.cases_long, names_from = Date, values_from = `Cumulative_Cases`) 
...
df.deaths <- pivot_wider(df.deaths_long, names_from = Date, values_from = `Cumulative_Deaths`) 
...
# Remove states with no cases ever 
...
n.2wk <- cbind.data.frame(ccount[,1:5], temp)
remove(temp)
...
ds$epi.curve.map.cat <- ifelse(ds$rebound == "rebound", "rebound", ds$epi.curve.cat)
...
alldata <- dplyr::bind_rows(purrr::map2(.x = sort(rep(c(5,10,15,20),4)), 
                     .y = c(rep(c(.01,.05,.1,.5),4)), .f = ~hotspot("Case", .x, .y))) 
